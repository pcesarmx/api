...
class GruntLambdaDeploy {
	...
	constructor(grunt, done, __dirnameRoot) {
		...
		let environment = config.environment || 'devel';
		...
		function setupParams(api_id) {
			...
			for (let key in endpoints.resources) {
				...
				// 	"type": "string",
				// 	"required": true,
				// 	"default": resource.path + "/" + resource.path
				// };
				// t._operation = resource.path + "/" + resource.path;
				...
				if (endpoints.resources[key].path !== "/") {
					...
					for (let kp in _keysParams) {
						...
						if ("fb|nocache|extrainfo|_operation|isProd".indexOf(_keysParams[kp]) > -1)
							continue;

						t[_keysParams[kp]] = "$input.params('" + _keysParams[kp] + "')";
			...
			grunt.file.write(__dirnameRoot + '/dist/swagger.json', JSON.stringify(loacalSwaggerAPI, null, 4)); //
